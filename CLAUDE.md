...
작업이 실행 중일 때(에이전트 대기, AWS 리소스 propagation 등)
독립적인 다음 작업이 있으면 병렬로 진행한다.

병렬 가능 조건 (모두 충족해야 함):
- docs/interfaces.md 기준으로 서로 다른 모듈 경계에 있음
- 한쪽 결과물이 다른 쪽 입력으로 쓰이지 않음
- Supabase/AWS 등 공유 자원에 동시 write 하지 않음

병렬 작업 방식:
- 새 터미널 탭 + 새 브랜치로 분리
- 각 작업은 독립된 Claude Code 세션에서 진행
- 완료 후 서로 다른 PR로 머지
