...
- `analyze()` - Zone Intelligence Agent:
  - 위치: `src/cado/agents/zone_intelligence.py`
  - 시그니처: `analyze(zone_id: str, recent_status: list[dict]) -> str`
  - 입력: `recent_status`는 `derived_operational_status` 형태 dict 1~3개의 리스트. 각 dict는
    `zone_id`, `evaluated_at`, `heat_risk_score`, `cooling_stress_score`, `zone_imbalance_score`,
    `status_level` 키를 포함해야 함.
  - 출력: 1~2문장 자연어 분석 문자열 ("이 zone이 왜 이 상태인지" 사람이 읽을 수 있는 설명).
    Anthropic API 호출 실패 시 정확히 `"분석 일시 불가"` 문자열 반환.
  - 사용 모델: `claude-sonnet-5` (Claude 5 세대 Sonnet 모델, max_tokens=150으로 응답 길이 제한)
  - 프롬프트 원칙: 수치 나열이 아닌 해석 제공, heuristic indicator 성격을 응답에 암시,
    1~2문장으로 응답 제한
  - 테스트: `tests/test_zone_intelligence.py` (mock 기반 유닛 테스트 6개 +
    `@pytest.mark.integration`으로 분리된 실제 API 호출 테스트 2개. mock 테스트만 기본 실행:
    `uv run pytest tests/test_zone_intelligence.py -v`)
